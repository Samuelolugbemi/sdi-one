...
export default async function Page() {
  const connector = await prisma.integrationConnector.findUnique({ where: { key: 'fuel-system' }, include: { mappings: true, runs: { orderBy: { startedAt: 'desc' }, take: 20 }, logs: { orderBy: { createdAt: 'desc' }, take: 20 } } });
  const mappings = connector?.mappings ?? [];
  const runs = connector?.runs ?? [];
  const logs = connector?.logs ?? [];
  return <AppShell>
    <PageHeader title="Fuel System Configuration" description="Fuel card/transaction connector for fuel spend, gallons, MPG and exception monitoring." actions={<Link className="btn btn-primary" href="/dashboard/integration-platform/connectors/fuel-system">Connector Overview</Link>} />
