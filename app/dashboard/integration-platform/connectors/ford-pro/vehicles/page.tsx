...
export default async function Page() {
  const connector = await prisma.integrationConnector.findUnique({ where: { key: 'ford-pro' }, include: { mappings: true, runs: { orderBy: { startedAt: 'desc' }, take: 20 }, logs: { orderBy: { createdAt: 'desc' }, take: 20 } } });
  const mappings = connector?.mappings ?? [];
  const runs = connector?.runs ?? [];
  const logs = connector?.logs ?? [];
  return <AppShell>
    <PageHeader title="Ford Pro Vehicle Objects" description="OEM fleet connector for vehicles, odometer, location and diagnostics." actions={<Link className="btn btn-primary" href="/dashboard/integration-platform/connectors/ford-pro">Connector Overview</Link>} />
